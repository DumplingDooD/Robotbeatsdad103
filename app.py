...
# Turn DEBUG on to show a diagnostics panel and to auto-clear daily cache at runtime
DEBUG = True
...
def fetch_with_ytdlp(video_id: str):
    ...
    try:
        ...
        ydl_opts = {
            ...
            "subtitlesformat": "vtt",
            "subtitleslangs": ["en", "en.*", "en-US", "en-GB"],  # match more keys
            "extractor_retries": 3,
            "sleep_interval_requests": 0.5,
            "http_headers": {"User-Agent": "Mozilla/5.0"},
            # Android client often exposes timedtext more consistently
            "extractor_args": {"youtube": {"player_client": ["android"]}},
            # If age/region gated later, add a cookies file:
        ...
        info["steps"].append(f"subs_keys:{list(subs.keys())[:10]}")
        info["steps"].append(f"auto_keys:{list(autos.keys())[:10]}")

        def pick_track(tracks_dict):
            # Prefer EN; else first available
            for lang in ("en", "en-US", "en-GB"):
                lst = tracks_dict.get(lang)
                if lst and lst[0].get("url"):
                    return lang, lst[0]["url"]
            for lang, lst in tracks_dict.items():
                if lst and lst[0].get("url"):
                    return lang, lst[0]["url"]
            return None, None

        lang, url, kind = None, None, "human"
        lang, url = pick_track(subs)
        if not url:
            kind = "auto"
            lang, url = pick_track(autos)

        if not url:
            info["steps"].append("no_caption_tracks_found")
            return None, None, False, info
    ...
    for channel_id, name in YOUTUBERS.items():
        try:
            ...
            if tx:
                ...
                transcript_note = (
                    f"(auto-translated from {src_lang})" if was_translated and src_lang
                    else f"(lang: {src_lang})" if src_lang else ""
                )
            else:
...
# Auto-bypass cache while DEBUG is on (so you see changes instantly)
if DEBUG:
    try:
        fetch_and_analyze_daily.clear()
    except Exception:
