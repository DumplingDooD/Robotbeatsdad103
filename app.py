...
DEBUG = True  # Show debug panel for captions/transcripts
...
MACRO_TERMS = {"cpi", "inflation", "jobs", "nonfarm", "payrolls", "pce", "core", "fomc", "fed", "rate", "hike", "cut",
...
def extract_entities(text: str):
    ...
    for sent in split_sentences(text):
        if re.search(LEVEL_NEAR, sent):
            ...
            pieces = []
            if price_hits:
                pieces.append(" ".join(price_hits[:3]))
            if pct_hits:
                pieces.append(" ".join(pct_hits[:3]))
...
def fetch_with_ytdlp(video_id: str):
    ...
    try:
        ...
        for kset, kind in ((subs, "human"), (autos, "auto")):
            for lang in ("en", "en-US", "en-GB"):
                if lang in kset:
                    url = kset[lang][0]["url"]
                    r = requests.get(url, timeout=30, headers=USER_AGENT)
                    if r.ok:
                        return vtt_to_text(r.text), lang, False, info
        return None, None, False, info
...
def fetch_transcript_text(video_id: str):
    ...
    return txt, lang, trans, {"yta": info1, "ytdlp": info2}
    ...
    for channel_id, name in YOUTUBERS.items():
        try:
            ...
            if tx:
                ...
                transcript_note = ""
            rows.append({
                "Name": name, "Video Title": video_title, "Published": published, "URL": video_url,
                "Summary": summary, "Sentiment": sentiment_icon, "KeyPoints": bullets,
                "Entities": ents, "TranscriptNote": transcript_note, "VideoID": video_id
            })
        except Exception as e:
            rows.append({
                "Name": name, "Video Title": "Unavailable", "Published": "", "URL": "",
                "Summary": f"Error fetching latest video: {e}", "Sentiment": "🟣 Unknown",
                "KeyPoints": [], "Entities": {"tickers": [], "macro": [], "actions": [], "levels": []},
                "TranscriptNote": "", "VideoID": None
            })
            time.sleep(0.4)
    return pd.DataFrame(rows), datetime.now(timezone.utc).isoformat(), debugs
...
if df.empty:
    ...
    for _, row in df.iterrows():
        ...
        if ents["levels"]: chips.append("**Levels/Calls:** " + " | ".join(ents["levels"]))
        if chips:
